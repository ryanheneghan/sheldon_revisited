...
source("./R/processing_scripts/residual_plot_function.R")

# Source interpolation function for global abundance/biomass
source("./R/processing_scripts/interpolate_global_abund_biomass.R")

##########################################################
## STEP 1: FIT STATISTICAL MODEL TO BIOMASS OBSERVATIONS
##########################################################
print("Fitting macrozoo biomass statistical model")
...
print("Plotting main effects plots for global macrozoo biomass statistical model")
...
print("Summary figures of partial residuals of macrozoo biomass stat model saved to ./figures/suppfig6_residuals_macrozoo_biomass.png")

##########################################################
## STEP 2: PREDICT GLOBAL MACROZOOPLANKTON BIOMASS AND 
##         CALCULATE UNCERTAINTY
##########################################################
print("Calculating global macrozoo biomass")
...
write.csv(macrozoo_predictions, './output/global_map_data/macrozoo_predictions.csv', row.names = FALSE)

print("Global map data of macrozoo biomass saved to ./output/global_map_data/macrozoo_predictions.csv")


## Calculate uncertainty of estimate using bootstrap sampling
print("Calculating total macrozoo biomass uncertainty")
if(run_bootstrap == TRUE){ # This takes about 10 minutes, you can just run the else statement to get the result, then skip the loop
n <- 1000 
...
##########################################################
print("Saving global total macrozoo biomass estimate and uncertainty")
...
summary_biomass_table_top200 <- read.csv("./output/summary_output/summary_tables/summary_biomass_table_top200.csv")
summary_biomass_table_top200[8, c(2:4)] <- tot_macrozoo_biom_upper200
write.csv(summary_biomass_table_top200, file = "./output/summary_output/summary_tables/summary_biomass_table_top200.csv", row.names = FALSE)
...
print("Total global macrozoo biomass and uncertainty estimates saved to tables in ./output/summary_output/summary_tables/")
